    ...
    <div className="bg-transparent min-h-screen">
      ...
      <div className="bg-slate-900 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl font-extrabold text-[#d5a46d] tracking-tight">Client Testimonials</h1>
            ...
            {testimonials.map((testimonial) => (
              <div key={testimonial.id} className="bg-slate-50 p-8 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex text-yellow-400 mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className={`h-5 w-5 ${i < (testimonial.rating || 0) ? 'fill-current' : 'text-slate-300'}`} />
                  ))}
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <p className="text-sm font-semibold text-slate-900">{testimonial.client_name || 'Anonymous'}</p>
